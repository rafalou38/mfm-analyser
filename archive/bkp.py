...
bufPhi = np.zeros(HISTORY * 10, dtype=np.float64)
bufA = np.zeros(HISTORY * 10, dtype=np.float64)
...
class MainPlot:
    ...
    def draw(self, t,x,y,ttl):
        # v = lk.get_view()
        self.curve0.setData(t, x * max(x_max, y_max))
        # self.curve1.setData(t, y)
        # self.curvez.setData(t, ttl * max(x_max, y_max))
        return
        # Find nan range
        ...
        # alpha = 0.99
        # I = np.zeros_like(I_raw)
        # Q = np.zeros_like(Q_raw)
        # for i in range(1, len(I_raw)):
        #     I[i] = alpha * I[i-1] + (1-alpha) * I_raw[i]
        #     Q[i] = alpha * Q[i-1] + (1-alpha) * Q_raw[i]
        ...
        # self.curve0.setData(t, A * max(x_max, y_max))
        # self.curve0.setData(t, I_raw * max(x_max, y_max))

        # print(I_raw)
        # self.curve1.setData(t, I * max(x_max, y_max))
    ...
    def show_peaks(self, viewt_filt, data):
        pass
        # # Remove old peak items
        ...
        # # Show peaks:
        # peaks, _ = find_peaks(data, height=0.01, prominence=0.02, distance=20)
        # print(len(peaks), len(edges))
        # for p in peaks:
        #     t = viewt_filt[p]
        #     item = pg.LineSegmentROI([[t, -0.02], [t, 0.02]], pen='r')
        #     self.plot.addItem(item)
        #     self.peak_items.append(item)

def lock_in(fs, f0):
    s = np.sin(2 * np.pi * f0 * np.arange(len(buf0)).roll(-ptr) / fs)
    c = np.cos(2 * np.pi * f0 * np.arange(len(buf0)).roll(-ptr) / fs)
    return s
...
class lock_in:
    def __init__(
        ...
        self.buf_x = np.zeros(history, dtype=np.float64)
        self.buf_y = np.zeros(history, dtype=np.float64)
        self.buf_ttl = np.zeros(history, dtype=np.int8)
        self.buf_t = np.zeros(history, dtype=np.float64)

        # =========================
        # LOCK-IN OUTPUTS
        # =========================

        self.buf_I = np.zeros(history, dtype=np.float64)
        self.buf_Q = np.zeros(history, dtype=np.float64)
        self.buf_phi = np.zeros(history, dtype=np.float64)
        self.buf_amp = np.zeros(history, dtype=np.float64)
    ...
    def read_serial(self):
        ...
        x = samples["x"].astype(np.float64)
        y = samples["y"].astype(np.float64)
        ttl = samples["z"].astype(np.int8)
        t = samples["t"].astype(np.float64) * 1e-6
        ...
        #     print(f"Lock-in initialized @ fs = {fs:.2f} Hz")

        # # =============
        # #  TTL -> Sine (reconstruction de phase)
        # # =============
        ...
        # # No edge -> continue previous phase
        # if len(edges) == 0:
        #     print("damned")
        #     # # estimate frequency from previous block
        #     # # fallback to last known phase increment
        ...
        #     #     self.phase_acc += dphi
        # else:
        #     # interpolate phase between edges
        ...
        self._store(self.buf_x, x)
        self._store(self.buf_y, y)
        self._store(self.buf_ttl, ttl)
        self._store(self.buf_t, t)

        # self._store(self.buf_I, I)
        # self._store(self.buf_Q, Q)
        # self._store(self.buf_phi, phi)
        # self._store(self.buf_amp, amp)

        return True

    def _store(self, buf, values):

        n = len(values)

        if self.ptr + n < self.HISTORY:
            buf[self.ptr:self.ptr+n] = values
        else:

            self.ready = True

            k = self.HISTORY - self.ptr

            buf[self.ptr:] = values[:k]
            buf[:n-k] = values[k:]
    ...
    def update(self):
        ...
        # rewind stream one frame
        self.proc.stdout.seek(
            -self.FRAME_BYTES,
            1
        )
        ...
        self.ptr = (self.ptr + n) % self.HISTORY

        return ok

    def view(self, buf):
        return np.roll(buf, -self.ptr)

    def get_view(self):

        return {
            "t": self.view(self.buf_t),
            "x": self.view(self.buf_x),
            "y": self.view(self.buf_y),
            "ttl": self.view(self.buf_ttl),
            "I": self.view(self.buf_I),
            "Q": self.view(self.buf_Q),
            "phi": self.view(self.buf_phi),
            "amp": self.view(self.buf_amp),
        }
